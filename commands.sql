...
-- Create CalculateTotalPrice stored procedure
DELIMITER //
CREATE PROCEDURE CalculateTotalPrice(IN basePrice DECIMAL(10, 2), IN quantity INT)
BEGIN
    -- Your calculation logic here
    SELECT basePrice * quantity AS TotalPrice;
END //
DELIMITER ;

-- Create CalculateDiscountedPrice stored procedure
DELIMITER //
CREATE PROCEDURE CalculateDiscountedPrice(IN originalPrice DECIMAL(10, 2), IN discountPercentage DECIMAL(5, 2))
BEGIN
    -- Your calculation logic here
    SELECT originalPrice - (originalPrice * (discountPercentage / 100)) AS DiscountedPrice;
END //
DELIMITER ;
